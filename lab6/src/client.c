...
uint64_t defineSocket(struct ServersSegment* args)
{
    // sozdaem socket
...
int main(int argc, char **argv) {
  uint64_t k = -1;
  uint64_t mod = -1;
  char servers[255] = {'\0'}; // TODO: explain why 255: ограничение линукса на длинну имени файла (в винде такой-же вроде) 
  ...
  for (int i = 0; i < servers_num; i++)
    {
        ...
        serversSegment[i].server.sin_addr.s_addr = *((unsigned long*)hostname->h_addr_list[0]);

        serversSegment[i].begin = 1 + i * segments;
        serversSegment[i].end = ((i != servers_num - 1) ? (i + 1) * segments : k);
        serversSegment[i].mod = mod;

        pthread_create(&threads[i], NULL, defineSocket, (void*)&serversSegment[i]);
  }
  // правильно лочим/анлочим, чтобы все работало
